...
use crate::{
    game::world::{
        block::block::Block,
        chunk::{Chunk, ChunkBlockCoordinate, ChunkCoordinate, CHUNK_HEIGHT, CHUNK_SIZE},
    },
    render::world::block::block::BlockRenderer,
};
    ...
    pub fn render(
        ...
        if self.model_cache.get(&chunk.position).is_none() {
            self.model_cache.insert(
                chunk.position,
                self.generate_model(chunk, north_chunk, south_chunk, east_chunk, west_chunk),
            );
        }
            ...
            for y in 0..CHUNK_HEIGHT {
                for z in 0..CHUNK_SIZE {
                    ...
                    let south_block = if z > 0 {
                        ...
                        match south_chunk {
                            Some(chunk) => {
                                chunk.get_block(ChunkBlockCoordinate::new(x, y, CHUNK_SIZE - 1))
                            }
                            None => Block::Air,
                        }
                    };
                    ...
                    let west_block = if x > 0 {
                        ...
                        match west_chunk {
                            Some(chunk) => {
                                chunk.get_block(ChunkBlockCoordinate::new(CHUNK_SIZE - 1, y, z))
                            }
                            None => Block::Air,
                        }
                    };
